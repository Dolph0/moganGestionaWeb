...
const Tab = (props: TabProps) => {
  return (
    <div
      className={`border-brand-gray-20 border bg-brand-gray-25 px-4 md:px-6 py-6 md:py-8`}
    >
      <article
  ...
  return (
    <ul className={`flex`}>
      {props.tabs.map((item) => (
        ...
        >
          <button
            className={`px-4 text-sm xl:text-[15px] py-4 h-full  group-[.active]:text-brand-green-100 group-[.active]:border-x group-[.active]:border-t group-[.active]:border-brand-gray-20 group-[.active]:bg-brand-gray-25 font-extrabold text-brand-blue-100 max-w-[180px] text-left leading-tight `}
